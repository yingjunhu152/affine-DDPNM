...
界面空间 `{1,s,t}⊗n`（仅法向 × 三项标量，3 模/面，432 未知量）：均匀 27 球上 L²(u)=8.2%、H¹=23.8%、L²(p)=1.8%、通量=2.4%（Classic 21.1% / Affine 5.6% 同网格四方法同次运行）。

- **法向线性模是主导机制**：从 Classic 到 Affine 的全部绝对改善中，W₁ₙ 单独贡献 ~84%（L² 21.1%→8.2%）；经典瓶颈是「界面上法向牵引只能取常数」（压力梯度的面内投影无法表达），不是代表实体个数。
- **切向线性模只做收尾**：8.2%→5.6%、通量 2.4%→0.32%；且 pL² 上无收益（2.2% vs 1.8%，反升）——切向模的作用是让两侧法向应力面内变化在 Schur 装配中精确配对。
- **成本**：W₁ₙ 离线 7.4 s ≈ Classic（Affine 9.2 s）、未知量为 Affine 1/3（432 vs 1,296）、在线 0.022 s。精度预算 ~8% 时 W₁ₙ 性价比更高；要 0.3% 流量精度必须上 Affine。
