...
## Standalone archive (2026-08-09)

This project now lives in `20260810report/` and runs standalone from there —
**no `PYTHONPATH` needed**. The entry scripts
(`run_ddpnm_2d.py`, `run_adaptive_ddpnm_2d.py`, `run_strict_error_2d.py`,
`audit_mesh.py`) prepend the parent directory (the archive root, which holds
`ddpnm_core/` and `postprocess/`) to `sys.path`, exactly like the 3-D projects.

```bash
cd 20260810report/ddpnm_2d_random_porous
export PATH="/d/Miniconda3/envs/fenicsx:/d/Miniconda3/envs/fenicsx/Library/bin:$PATH"
/d/Miniconda3/envs/fenicsx/python.exe run_ddpnm_2d.py --out-dir outputs/enriched
/d/Miniconda3/envs/fenicsx/python.exe verify_results.py outputs/enriched/report.json
```

Verified 2026-08-09: all eight invariant checks pass; vertex-sampled errors
`velocity=4.455%`, `pressure=0.951%` match the original baseline bit-for-bit.

### Fix record

`PolynomialNormalBasis.global_keys` returned 2-tuples `(interface_id, degree)`,
but the unified `ddpnm_core.assembler.InterfaceAssembler` matches 3-tuples
`(interface_id, component, polynomial)` (the 3-D bases' contract). Every key
missed, the Schur matrix assembled to all zeros, and all traction coefficients
came back zero (velocity error 1588%). Fixed in `ddpnm2d/basis_2d.py`
(`global_keys` now returns `(interface_id, "normal", "P0"/"P1")`) and
`ddpnm2d/solver.py` (coefficient/flux lookup keys). `HierarchyBasis` keys are
unchanged — the adaptive driver assembles its own system with its own keys.
