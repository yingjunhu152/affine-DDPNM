...
Pythagorean decomposition; and (iii)~the volume error separates into
a C-DD discretisation component and an interface-modal-truncation
component.  The latter is identified exactly in the broken energy and
broken-$H^1$ measures; in $L^2$, an exact two-sided triangle bound is
combined with a one-sided Poincar\'e--Friedrichs estimate.
...
\end{align}
On the full broken product space the last two gradient-based quantities
are seminorms.  If every $\pore{i}$ is connected and Lipschitz and
...
\begin{enumerate}[leftmargin=*,itemsep=4pt]
  ...
  \item \textbf{The four-space diamond is diagnostic, not a unique
    ...
    The two middle spaces $\Wspace_{0v}$ and $\Wspace_{1n}$ are
    nonnested, so the two Pythagorean paths in
    Remark~\ref{rem:four-space} provide path-dependent marginal gains.
    No unique attribution of total error to ``tangential'' and
    ``normal-linear'' effects follows from orthogonality alone.
