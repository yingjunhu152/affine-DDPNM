...
def _classify_slice_grid(
    slice_points, slice_triangles, tlabels, z_value, spheres, radii, n=400,
):
    ...
    ``tlabels`` is the per-triangle subdomain label array (T,).
    """
...
def _smooth_slice_grid(
    ...
    slice_triangles=None, tri_labels=None,
):
    ...
    When ``tri_labels`` (per-triangle labels) is given, the vertex set of a
    subdomain is taken as the vertices of all triangles carrying that label,
    which includes the interface vertices themselves - without this, labels
    derived per vertex would drop boundary vertices from one side and the
    interpolant hull would shrink, producing white NaN bands along the
    interfaces (visible e.g. on the Cartesian uniform geometry).
    """
    grid = np.column_stack((X.ravel(), Y.ravel()))
    Z = np.full(len(grid), np.nan)
    if tri_labels is not None and slice_triangles is not None:
        labels = np.unique(tri_labels)
        verts_of = {
            int(k): np.unique(slice_triangles[tri_labels == k]) for k in labels
        }
    else:
        labels = np.unique(vertex_labels)
        verts_of = {int(k): np.flatnonzero(vertex_labels == k) for k in labels}
    for k in labels:
        verts = verts_of[int(k)]
        if len(verts) < 4:
            continue
        interpolator = LinearNDInterpolator(slice_points[verts][:, :2], values[verts])
...
def _subdomain_boundary_lines(slice_points, slice_triangles, tlabels):
...
def slice_field_grids(data, n=400, spheres=None, radii=None, vertex_labels=None,
                      tri_labels=None, global_interp=False, boundary_lines=None):
    ...
    cell-label arrays).  ``tri_labels`` is the preferred way to provide
    subdomain labels: per-triangle labels (from triangle centroids or parent
    cells) keep the interface vertices in both adjacent subdomains' vertex
    sets, which avoids white NaN bands along the interfaces; when omitted,
    labels fall back to ``vertex_labels`` of each triangle's first vertex.
    """
    ...
    if vertex_labels is None:
        if "cell_labels" in data.files:
            vertex_labels = np.asarray(data["cell_labels"])[
                data["classic_error_slice_parent_cells"]
            ]
        else:  # archives without cell arrays (Uniform-27): unused when
            vertex_labels = np.zeros(len(slice_points), dtype=int)  # tri_labels given
    if tri_labels is None:
        tri_labels = vertex_labels[slice_triangles][:, 0]
    if global_interp:
        # The Uniform-27 slice triangulation is not conforming to the grid
        # partition (triangles cross the partition planes, no shared edge
        # joins two subdomains), so per-subdomain interpolation cannot work:
        # interpolate globally over all slice vertices (sphere holes stay
        # masked via `usable`) and let the caller supply analytic boundary
        # traces instead of triangulation-derived ones.
        tri_labels = np.zeros(len(slice_triangles), dtype=int)
    ...
    for key, suffix in (("W0n", "classic"), ("W1n", "normal_linear"), ("W1v", "affine")):
        ...
        raw[f"err_{key}"] = np.abs(np.linalg.norm(u_dd, axis=1) - speed_ref)

    X, Y, cell_of_point, usable = _classify_slice_grid(
        slice_points, slice_triangles, tri_labels, z_value, spheres, radii, n=n
    )
    grids = {
        key: _smooth_slice_grid(
            values, slice_points, vertex_labels, X, Y, cell_of_point, usable, n=n,
            slice_triangles=slice_triangles, tri_labels=tri_labels,
        )
    ...
    if boundary_lines is not None:
        ...
        boundaries = _subdomain_boundary_lines(slice_points, slice_triangles, tri_labels)
    return {
        "grids": grids,
        "X": X,
        "Y": Y,
