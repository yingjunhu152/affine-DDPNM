...
class LocalStokesOperator:
    ...
    The default bilinear form is ::

        a = viscosity · ⟨∇u, ∇v⟩ − p∇·v − q∇·u − stab · p·q

    ``viscous_form="symmetric"`` instead uses
    ``2 viscosity · sym(∇u):sym(∇v)``.  Both use zero Dirichlet BCs for
    velocity on ``WALL_TAG`` facets.  The
...
def build_local_stokes_operator(
    ...
    W = mixed_stokes_space(submesh, velocity_degree=velocity_degree)
    ...
    h = ufl.CellDiameter(submesh)

    if viscous_form == "gradient":
        viscous_term = ufl.inner(ufl.grad(u), ufl.grad(v))
    elif viscous_form == "symmetric":
        viscous_term = 2.0 * ufl.inner(ufl.sym(ufl.grad(u)), ufl.sym(ufl.grad(v)))
    else:
        raise ValueError(f"Unknown viscous form: {viscous_form}")

    a = (
        viscosity * viscous_term * dx
        - p_var * ufl.div(v) * dx
        - q * ufl.div(u) * dx
        - pressure_stabilization * p_var * q * dx
        - pressure_gradient_stabilization * h * h
        * ufl.inner(ufl.grad(p_var), ufl.grad(q)) * dx
    )
    a_form = fem.form(a)
