...
def mixed_stokes_space(msh: dmesh.Mesh, velocity_degree: int = 2) -> fem.FunctionSpace:
    """Mixed ``[P_k]^d-P1`` space, Taylor–Hood when ``k=2``.

    Equal-order ``k=1`` is supported for memory-constrained digital-core
    benchmarks and must be paired with pressure-gradient stabilization.
    """
    gdim = msh.geometry.dim
    cell = msh.basix_cell()
    if velocity_degree not in (1, 2):
        raise ValueError("velocity_degree must be 1 or 2")
    velocity = element("Lagrange", cell, velocity_degree, shape=(gdim,))
...
def solve_reference(
    ...
    iterative_progress_interval: int = 0,
    iterative_raise: bool = True,
):
    ...
    W = mixed_stokes_space(msh, velocity_degree=velocity_degree)
    ...
    a = (
        ...
        - pressure_gradient_stabilization * h * h
        * ufl.inner(ufl.grad(p), ufl.grad(q)) * dx
    )
    L = (
    ...
    if A.shape[0] < iterative_threshold:
        ...
        if block_size != gdim or len(velocity_to_mixed) % block_size:
            raise RuntimeError(
                f"Unexpected vector P{velocity_degree} layout in the FEM preconditioner."
            )
        ...
        def record_residual(value: float) -> None:
            ...
            if (
                iterative_progress_interval > 0
                and len(residual_history) % iterative_progress_interval == 0
            ):
                print(
                    f"      FEM GMRES iteration {len(residual_history)}: "
                    f"preconditioned residual={residual_history[-1]:.3e}",
                    flush=True,
                )
        ...
        solver_method = (
            f"SciPy GMRES with scalar-P{velocity_degree} ILU and exact P1-mass block preconditioner"
        )
    ...
    if relative_linear_residual > max(1.0e-8, 20.0 * iterative_rtol):
        converged = False
        solver_method = "(residual above tolerance) " + solver_method
        if iterative_raise:
            raise RuntimeError(
