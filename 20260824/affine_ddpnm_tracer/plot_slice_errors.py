#!/usr/bin/env python3
"""Slice-plane error fields for the affine-DDPNM tracer run (archived style).

Mimics the archived ``affine_ddpnm_3d_random_porous/plot_random_errors.py``
(``01_slice_error_fields.png``): the tetra mesh is cut at ``z = 0.5``, the
P1 vertex fields are evaluated on the slice, resampled on a regular grid
with per-subdomain interpolation and masked Gaussian smoothing (the jump
across Voronoi interfaces is preserved), sphere cross-sections are drawn as
cut-outs, interface traces as black lines, and every error panel gets an
independent 98th-percentile color scale with a sci-format colorbar.

Panels (2 x 3, z = 0.5):

- row 1: velocity error ``||u_ddpnm| - |u_FEM||`` per method;
- row 2: tracer concentration error ``|c_ddpnm - c_FEM|`` per method.

Also regenerates ``tracer_error_summary.png`` with the readable y-axis.
...
def plot_slice_error_fields(data, out_dir: Path) -> None:
    ...
    c_fem_slice = evaluate_p1_on_slice(
        points, tetrahedra, c_fem[:, None], slice_points, groups
    )[:, 0]
    ...
    for method in METHODS:
        ...
        c_dd = evaluate_p1_on_slice(
            points, tetrahedra, np.asarray(data[f"c_{key}"], dtype=float)[:, None],
            slice_points, groups,
        )[:, 0]
        fields[f"speed_err_{method}"] = np.abs(np.linalg.norm(u_dd, axis=1) - speed_ref)
        fields[f"c_err_{method}"] = np.abs(c_dd - c_fem_slice)
    ...
    for method in METHODS:
        entries.append(
            (
                grids[f"c_err_{method}"],
                "turbo", 0.0, limit(grids[f"c_err_{method}"]),
                f"{method}: $|c-c_{{\\mathrm{{FEM}}}}|$",
            )
        )
    ...
    fig.text(
        0.012, 0.28, "Tracer concentration error vs FEM",
        rotation=90, va="center", ha="center", fontsize=11, fontstyle="italic",
    )
    fig.text(
